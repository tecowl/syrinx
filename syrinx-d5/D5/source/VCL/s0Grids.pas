 ...
 * 
 * *****************  Version 13  *****************
 * User: Akima        Date: 02/02/06   Time: 19:25
 * Updated in $/source/D6VCLIntegrated
 * VER130ではConstsユニットに定義されていた変数を、VER140以降では独自に定
 * 義することにした。
 * 
 * *****************  Version 12  *****************
 * User: Akima        Date: 02/02/06   Time: 1:03
 * Updated in $/source/D5Integrated
 * UpdateTextメソッドにおいて、InplaceEditorのnilチェック、表示チェックを
 * 追加
 * 
 * *****************  Version 11  *****************
 * User: Akima        Date: 02/02/05   Time: 4:26
 * Updated in $/source/D5Integrated
 * Ts0CustomGrid.InvalidateEditorで、アクティブセルが移動してないのに
 * InplaceEditorMoveToを呼び出していたので、移動したかどうかを判断するよう
 * に修正。
 * 
 * *****************  Version 10  *****************
 ...
 * 
 * *****************  Version 3  *****************
 * User: Akima        Date: 01/09/06   Time: 21:23
 * Updated in $/source/D5Integrated
 * SelectionMovedをprotectedに変更
 * 
 * *****************  Version 2  *****************
 * User: Akima        Date: 01/07/30   Time: 10:19
 * Updated in $/source/p0control/p0grid
 * ・Ts0CustomGrid
 * CalcCoordFromPointメソッドをprivateからprotectedに変更。
 * Paintメソッドで使用するIsDrawableメソッドを追加。
 * インプレースエディタを指定したセルに表示するInplaceEditMoveメソッドを追
 * 加
 * ColCount、RowCountプロパティをpublicに変更
 * 
}
...
type
  ...
  { Ts0CustomGrid is an abstract base class that can be used to implement
    ...
    Protected members, for implementors of Ts0CustomGrid descendents
      DesignOptionBoost
  ...
  Ts0CustomGrid = class(TCustomControl)
  private
    FAnchor: TGridCoord;
...
//    FCanEditModify: Boolean;
  ...
  protected
    function CalcCoordFromPoint(X, Y: Integer;
      const DrawInfo: TGridDrawInfo): TGridCoord; virtual;
  private
  ...
  protected
    procedure GridRectToScreenRect(GridRect: TGridRect;
      var ScreenRect: TRect; IncludeLine: Boolean); virtual;
  ...
  protected
    procedure InplaceEditorMoveTo(ACol, ARow: Integer); virtual;
  ...
  protected
    ...
    { Add by AKM }
    function GetInplaceEdit: TWinControl; virtual;
    property Anchor: TGridCoord read FAnchor write FAnchor;
    property Current: TGridCoord read FCurrent;
    function IsDrawable(ACol, ARow: Integer): Boolean; virtual;
    procedure InplaceEditMove(ACol, ARow: Integer); virtual;
    function GetInplaceEditText(AControl: TWinControl): string; virtual;
    procedure SetInplaceEditText(AControl: TWinControl; const Value: string); virtual;
  protected
...
//    function CanEditModify: Boolean; dynamic;
    ...
    property InplaceEditor: TWinControl read GetInplaceEdit;
...
type
  Ts0CustomDrawGrid = class(Ts0CustomGrid)
  private
...
type
  Ts0DrawGrid = class(Ts0CustomDrawGrid)
  public
    property Canvas;
    property Col;
    property ColWidths;
...
type
  ...
  { Ts0CustomStringGrid adds to Ts0CustomDrawGrid the ability to save a string and associated
  ...
  Ts0CustomStringGrid = class;

  TStringGridStrings = class(TStrings)
  private
    FGrid: Ts0CustomStringGrid;
    FIndex: Integer;
  ...
  public
    constructor Create(AGrid: Ts0CustomStringGrid; AIndex: Longint);
  ...
  Ts0CustomStringGrid = class(Ts0CustomDrawGrid)
  private
    FData: Pointer;
    FRows: Pointer;
    FCols: Pointer;
...
constructor Ts0CustomGrid.Create(AOwner: TComponent);
const
...
destructor Ts0CustomGrid.Destroy;
begin
  //InplaceEdit.Free;
  inherited Destroy;
  FreeMem(FColWidths);
...
procedure Ts0CustomGrid.AdjustSize(Index, Amount: Longint; Rows: Boolean);
var
  NewCur: TGridCoord;
...
function Ts0CustomGrid.BoxRect(ALeft, ATop, ARight, ABottom: Longint): TRect;
var
  GridRect: TGridRect;
begin
  GridRect.Left := ALeft;
...
procedure Ts0CustomGrid.DoExit;
begin
  inherited DoExit;
  {
    ここでHideEditorを呼び出されると、困っちゃうっす
  }
  //if not (goAlwaysShowEditor in Options) then HideEditor;
end;

function Ts0CustomGrid.CellRect(ACol, ARow: Longint): TRect;
begin
  Result := BoxRect(ACol, ARow, ACol, ARow);
end;

function Ts0CustomGrid.CanEditAcceptKey(Key: Char): Boolean;
begin
  Result := True;
end;

function Ts0CustomGrid.CanGridAcceptKey(Key: Word; Shift: TShiftState): Boolean;
begin
  Result := True;
end;

{
function Ts0CustomGrid.CanEditModify: Boolean;
begin
  Result := FCanEditModify;
end;
}

function Ts0CustomGrid.CanEditShow: Boolean;
begin
...
function Ts0CustomGrid.IsActiveControl: Boolean;
var
  H: Hwnd;
...
function Ts0CustomGrid.GetEditMask(ACol, ARow: Longint): string;
begin
  Result := '';
end;

function Ts0CustomGrid.GetEditText(ACol, ARow: Longint): string;
begin
  Result := '';
end;

procedure Ts0CustomGrid.SetEditText(ACol, ARow: Longint; const Value: string);
begin
end;

function Ts0CustomGrid.GetEditLimit: Integer;
begin
  Result := 0;
end;

procedure Ts0CustomGrid.HideEditor;
begin
  FEditorMode := False;
  HideEdit;
end;

procedure Ts0CustomGrid.ShowEditor;
begin
  FEditorMode := True;
  UpdateEdit;
end;

procedure Ts0CustomGrid.ShowEditorChar(Ch: Char);
begin
  ShowEditor;
...
procedure Ts0CustomGrid.InvalidateEditor;
begin
  if (Col <> FInplaceCol) or (Row <> FInplaceRow) then
    InplaceEditorMoveTo(-1, -1);
  UpdateEdit;
end;

procedure Ts0CustomGrid.ReadColWidths(Reader: TReader);
var
  I: Integer;
begin
  with Reader do
...
procedure Ts0CustomGrid.ReadRowHeights(Reader: TReader);
var
  I: Integer;
begin
  with Reader do
...
procedure Ts0CustomGrid.WriteColWidths(Writer: TWriter);
var
  I: Integer;
begin
  with Writer do
...
procedure Ts0CustomGrid.WriteRowHeights(Writer: TWriter);
var
  I: Integer;
begin
  with Writer do
...
procedure Ts0CustomGrid.DefineProperties(Filer: TFiler);

  function DoColWidths: Boolean;
  begin
    if Filer.Ancestor <> nil then
      Result := not CompareExtents(Ts0CustomGrid(Filer.Ancestor).FColWidths, FColWidths)
    else
  ...
  begin
    if Filer.Ancestor <> nil then
      Result := not CompareExtents(Ts0CustomGrid(Filer.Ancestor).FRowHeights, FRowHeights)
    else
...
procedure Ts0CustomGrid.MoveColumn(FromIndex, ToIndex: Longint);
var
  Rect: TGridRect;
begin
...
procedure Ts0CustomGrid.ColumnMoved(FromIndex, ToIndex: Longint);
begin
end;

procedure Ts0CustomGrid.MoveRow(FromIndex, ToIndex: Longint);
begin
...
procedure Ts0CustomGrid.RowMoved(FromIndex, ToIndex: Longint);
begin
end;

function Ts0CustomGrid.MouseCoord(X, Y: Integer): TGridCoord;
var
...
procedure Ts0CustomGrid.MoveColRow(ACol, ARow: Longint; MoveAnchor,
  Show: Boolean);
begin
  MoveCurrent(ACol, ARow, MoveAnchor, Show);
end;

function Ts0CustomGrid.SelectCell(ACol, ARow: Longint): Boolean;
begin
  Result := True;
end;

procedure Ts0CustomGrid.SizeChanged(OldColCount, OldRowCount: Longint);
begin
end;

function Ts0CustomGrid.Sizing(X, Y: Integer): Boolean;
var
  DrawInfo: TGridDrawInfo;
  State: TGridState;
  Index: Longint;
...
procedure Ts0CustomGrid.TopLeftChanged;
begin
  InplaceEditMove(Col, Row);
//  if FEditorMode and (InplaceEditor <> nil) then InplaceEditor.UpdateLoc(CellRect(Col, Row));
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if not IsDrawable(CurCol, CurRow) then
          begin
...
begin
  ...
  begin
    ...
    begin
      ...
      LineColor := Self.GridLineColor; //clSilver;
      ...
      DrawLines(
        ...
        goFixedVertLine in Options,
        LeftCol, 0,
        [Horz.FixedBoundary, 0, Horz.GridBoundary, Vert.FixedBoundary],
        clBlack,
        FixedColor
      );
      ...
      DrawLines(
        ...
        goFixedVertLine in Options,
        0, TopRow,
        [0, Vert.FixedBoundary, Horz.FixedBoundary, Vert.GridBoundary],
        clBlack,
        FixedColor
      );

      DrawLines(
        goHorzLine in Options,
...
function Ts0CustomGrid.CalcCoordFromPoint(X, Y: Integer;
  const DrawInfo: TGridDrawInfo): TGridCoord;
...
procedure Ts0CustomGrid.CalcDrawInfo(var DrawInfo: TGridDrawInfo);
begin
  CalcDrawInfoXY(DrawInfo, ClientWidth, ClientHeight);
end;

procedure Ts0CustomGrid.CalcDrawInfoXY(var DrawInfo: TGridDrawInfo;
  UseWidth, UseHeight: Integer);
...
procedure Ts0CustomGrid.CalcFixedInfo(var DrawInfo: TGridDrawInfo);
...
function Ts0CustomGrid.CalcMaxTopLeft(const Coord: TGridCoord;
  const DrawInfo: TGridDrawInfo): TGridCoord;
...
procedure Ts0CustomGrid.CalcSizingState(X, Y: Integer; var State: TGridState;
  var Index: Longint; var SizingPos, SizingOfs: Integer;
...
procedure Ts0CustomGrid.ChangeGridOrientation(RightToLeftOrientation: Boolean);
var
  Org: TPoint;
  Ext: TPoint;
begin
...
procedure Ts0CustomGrid.ChangeSize(NewColCount, NewRowCount: Longint);
var
...
procedure Ts0CustomGrid.ClampInView(const Coord: TGridCoord);
var
  DrawInfo: TGridDrawInfo;
...
procedure Ts0CustomGrid.DrawSizingLine(const DrawInfo: TGridDrawInfo);
var
  OldPen: TPen;
begin
  OldPen := TPen.Create;
...
procedure Ts0CustomGrid.DrawMove;
var
  OldPen: TPen;
  Pos: Integer;
  R: TRect;
...
procedure Ts0CustomGrid.FocusCell(ACol, ARow: Longint; MoveAnchor: Boolean);
begin
...
procedure Ts0CustomGrid.GridRectToScreenRect(GridRect: TGridRect;
...
procedure Ts0CustomGrid.Initialize;
begin
...
procedure Ts0CustomGrid.InvalidateCell(ACol, ARow: Longint);
var
  Rect: TGridRect;
begin
  Rect.Top := ARow;
...
procedure Ts0CustomGrid.InvalidateCol(ACol: Longint);
var
  Rect: TGridRect;
begin
  if not HandleAllocated then Exit;
...
procedure Ts0CustomGrid.InvalidateRow(ARow: Longint);
var
  Rect: TGridRect;
begin
  if not HandleAllocated then Exit;
...
procedure Ts0CustomGrid.InvalidateGrid;
begin
  Invalidate;
end;

procedure Ts0CustomGrid.InvalidateRect(ARect: TGridRect);
var
  InvalidRect: TRect;
begin
  if not HandleAllocated then Exit;
...
procedure Ts0CustomGrid.ModifyScrollBar(ScrollBar, ScrollCode, Pos: Cardinal;
  UseRightToLeft: Boolean);
var
...
procedure Ts0CustomGrid.MoveAdjust(var CellPos: Longint; FromIndex, ToIndex: Longint);
var
  Min, Max: Longint;
begin
...
procedure Ts0CustomGrid.MoveAnchor(const NewAnchor: TGridCoord);
var
  OldSel: TGridRect;
begin
...
procedure Ts0CustomGrid.MoveCurrent(ACol, ARow: Longint; MoveAnchor,
  Show: Boolean);
var
  OldSel: TGridRect;
...
procedure Ts0CustomGrid.MoveTopLeft(ALeft, ATop: Longint);
var
  OldTopLeft: TGridCoord;
begin
...
procedure Ts0CustomGrid.ResizeCol(Index: Longint; OldSize, NewSize: Integer);
begin
  InvalidateGrid;
end;

procedure Ts0CustomGrid.ResizeRow(Index: Longint; OldSize, NewSize: Integer);
begin
  InvalidateGrid;
end;

procedure Ts0CustomGrid.SelectionMoved(const OldSel: TGridRect);
var
...
procedure Ts0CustomGrid.ScrollDataInfo(DX, DY: Integer;
...
procedure Ts0CustomGrid.ScrollData(DX, DY: Integer);
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
procedure Ts0CustomGrid.TopLeftMoved(const OldTopLeft: TGridCoord);
...
procedure Ts0CustomGrid.UpdateScrollPos;
var
  DrawInfo: TGridDrawInfo;
  MaxTopLeft: TGridCoord;
  GridSpace, ColWidth: Integer;
...
procedure Ts0CustomGrid.UpdateScrollRange;
var
...
{
function Ts0CustomGrid.CreateEditor: TInplaceEdit;
begin
  Result := TInplaceEdit.Create(Self);
end;
}

procedure Ts0CustomGrid.CreateParams(var Params: TCreateParams);
begin
...
procedure Ts0CustomGrid.KeyDown(var Key: Word; Shift: TShiftState);
var
...
procedure Ts0CustomGrid.KeyPress(var Key: Char);
begin
...
procedure Ts0CustomGrid.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  CellHit: TGridCoord;
  DrawInfo: TGridDrawInfo;
...
procedure Ts0CustomGrid.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  DrawInfo: TGridDrawInfo;
  CellHit: TGridCoord;
begin
...
procedure Ts0CustomGrid.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  DrawInfo: TGridDrawInfo;
  NewSize: Integer;
...
procedure Ts0CustomGrid.MoveAndScroll(Mouse, CellHit: Integer;
...
function Ts0CustomGrid.GetColWidths(Index: Longint): Integer;
begin
...
function Ts0CustomGrid.GetRowHeights(Index: Longint): Integer;
begin
...
function Ts0CustomGrid.GetGridWidth: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function Ts0CustomGrid.GetGridHeight: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function Ts0CustomGrid.GetSelection: TGridRect;
begin
  Result := GridRect(FCurrent, FAnchor);
end;

function Ts0CustomGrid.GetTabStops(Index: Longint): Boolean;
begin
  if FTabStops = nil then Result := True
  else Result := Boolean(PIntArray(FTabStops)^[Index + 1]);
end;

function Ts0CustomGrid.GetVisibleColCount: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function Ts0CustomGrid.GetVisibleRowCount: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
procedure Ts0CustomGrid.SetBorderStyle(Value: TBorderStyle);
begin
...
procedure Ts0CustomGrid.SetCol(Value: Longint);
begin
  if Col <> Value then FocusCell(Value, Row, True);
end;

procedure Ts0CustomGrid.SetColCount(Value: Longint);
begin
...
procedure Ts0CustomGrid.SetColWidths(Index: Longint; Value: Integer);
begin
  if FColWidths = nil then
    UpdateExtents(FColWidths, ColCount, DefaultColWidth);
...
procedure Ts0CustomGrid.SetDefaultColWidth(Value: Integer);
begin
...
procedure Ts0CustomGrid.SetDefaultRowHeight(Value: Integer);
begin
...
procedure Ts0CustomGrid.SetFixedColor(Value: TColor);
begin
...
procedure Ts0CustomGrid.SetFixedCols(Value: Integer);
begin
...
procedure Ts0CustomGrid.SetFixedRows(Value: Integer);
begin
...
procedure Ts0CustomGrid.SetEditorMode(Value: Boolean);
begin
  if not Value then
    HideEditor
  else
...
procedure Ts0CustomGrid.SetGridLineWidth(Value: Integer);
begin
...
procedure Ts0CustomGrid.SetLeftCol(Value: Longint);
begin
  if FTopLeft.X <> Value then MoveTopLeft(Value, TopRow);
end;

procedure Ts0CustomGrid.SetOptions(Value: TGridOptions);
begin
...
procedure Ts0CustomGrid.SetRow(Value: Longint);
begin
  if Row <> Value then FocusCell(Col, Value, True);
end;

procedure Ts0CustomGrid.SetRowCount(Value: Longint);
begin
...
procedure Ts0CustomGrid.SetRowHeights(Index: Longint; Value: Integer);
begin
  if FRowHeights = nil then
    UpdateExtents(FRowHeights, RowCount, DefaultRowHeight);
...
procedure Ts0CustomGrid.SetScrollBars(Value: TScrollStyle);
begin
...
procedure Ts0CustomGrid.SetSelection(Value: TGridRect);
var
  OldSel: TGridRect;
begin
  OldSel := Selection;
...
procedure Ts0CustomGrid.SetTabStops(Index: Longint; Value: Boolean);
begin
  if FTabStops = nil then
    UpdateExtents(FTabStops, ColCount, Integer(True));
  if Index >= ColCount then InvalidOp(SIndexOutOfRange);
...
procedure Ts0CustomGrid.SetTopRow(Value: Longint);
begin
  if FTopLeft.Y <> Value then MoveTopLeft(LeftCol, Value);
end;

procedure Ts0CustomGrid.HideEdit;
begin
  if InplaceEditor <> nil then
    try
      UpdateText;
...
procedure Ts0CustomGrid.UpdateEdit;
var
  s: string;
begin
  begin
    ...
    begin
      HideEdit;
      InplaceEditorMoveTo(Col, Row);
      s := GetEditText(Col, Row);
      SetInplaceEditText(InplaceEditor, s);
    end;
    if CanEditShow then
    begin
      InplaceEditMove(Col, Row);
...
procedure Ts0CustomGrid.UpdateText;
begin
  if InplaceEditor = nil then Exit;
  if not InplaceEditor.Visible then Exit;
  if (FInplaceCol <> -1) and (FInplaceRow <> -1) then
    SetEditText(FInplaceCol, FInplaceRow, GetInplaceEditText(InplaceEditor) );
end;

procedure Ts0CustomGrid.WMChar(var Msg: TWMChar);
begin
...
procedure Ts0CustomGrid.WMCommand(var Message: TWMCommand);
begin
  with Message do
  begin
...
procedure Ts0CustomGrid.WMGetDlgCode(var Msg: TWMGetDlgCode);
begin
...
procedure Ts0CustomGrid.WMKillFocus(var Msg: TWMKillFocus);
begin
  inherited;
  InvalidateRect(Selection);
...
procedure Ts0CustomGrid.WMLButtonDown(var Message: TMessage);
begin
  inherited;
//  if InplaceEditor <> nil then InplaceEditor.FClickTime := GetMessageTime;
end;

procedure Ts0CustomGrid.WMNCHitTest(var Msg: TWMNCHitTest);
begin
  DefaultHandler(Msg);
  FHitTest := ScreenToClient(SmallPointToPoint(Msg.Pos));
end;

procedure Ts0CustomGrid.WMSetCursor(var Msg: TWMSetCursor);
var
  DrawInfo: TGridDrawInfo;
  State: TGridState;
  Index: Longint;
...
procedure Ts0CustomGrid.WMSetFocus(var Msg: TWMSetFocus);
begin
  inherited;
  if (InplaceEditor = nil) or (Msg.FocusedWnd <> InplaceEditor.Handle) then
  begin
    InvalidateRect(Selection);
    UpdateEdit;
  end;
...
procedure Ts0CustomGrid.WMSize(var Msg: TWMSize);
begin
  inherited;
  UpdateScrollRange;
...
procedure Ts0CustomGrid.WMVScroll(var Msg: TWMVScroll);
begin
  ModifyScrollBar(SB_VERT, Msg.ScrollCode, Msg.Pos, True);
end;

procedure Ts0CustomGrid.WMHScroll(var Msg: TWMHScroll);
begin
  ModifyScrollBar(SB_HORZ, Msg.ScrollCode, Msg.Pos, True);
end;

procedure Ts0CustomGrid.CancelMode;
var
  DrawInfo: TGridDrawInfo;
begin
  try
...
procedure Ts0CustomGrid.WMCancelMode(var Msg: TWMCancelMode);
begin
  inherited;
  CancelMode;
end;

procedure Ts0CustomGrid.CMCancelMode(var Msg: TMessage);
begin
//  if Assigned(InplaceEditor) then InplaceEditor.WndProc(Msg);
  inherited;
  CancelMode;
end;

procedure Ts0CustomGrid.CMFontChanged(var Message: TMessage);
begin
//  if InplaceEditor <> nil then InplaceEditor.Font := Font;
  inherited;
end;

procedure Ts0CustomGrid.CMCtl3DChanged(var Message: TMessage);
begin
  inherited;
  RecreateWnd;
end;

procedure Ts0CustomGrid.CMDesignHitTest(var Msg: TCMDesignHitTest);
begin
  Msg.Result := Longint(BOOL(Sizing(Msg.Pos.X, Msg.Pos.Y)));
end;

procedure Ts0CustomGrid.CMWantSpecialKey(var Msg: TCMWantSpecialKey);
begin
  inherited;
  if (goEditing in Options) and (Char(Msg.CharCode) = #13) then Msg.Result := 1;
end;

procedure Ts0CustomGrid.TimedScroll(Direction: TGridScrollDirection);
var
...
procedure Ts0CustomGrid.WMTimer(var Msg: TWMTimer);
var
  Point: TPoint;
  DrawInfo: TGridDrawInfo;
...
procedure Ts0CustomGrid.ColWidthsChanged;
begin
  UpdateScrollRange;
  UpdateEdit;
end;

procedure Ts0CustomGrid.RowHeightsChanged;
begin
  UpdateScrollRange;
  UpdateEdit;
end;

procedure Ts0CustomGrid.DeleteColumn(ACol: Longint);
begin
  MoveColumn(ACol, ColCount-1);
  ColCount := ColCount - 1;
end;

procedure Ts0CustomGrid.DeleteRow(ARow: Longint);
begin
  MoveRow(ARow, RowCount - 1);
  RowCount := RowCount - 1;
end;

procedure Ts0CustomGrid.UpdateDesigner;
var
  ParentForm: TCustomForm;
begin
...
function Ts0CustomGrid.DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
...
function Ts0CustomGrid.DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
...
function Ts0CustomGrid.CheckColumnDrag(var Origin,
  Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function Ts0CustomGrid.CheckRowDrag(var Origin,
  Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function Ts0CustomGrid.BeginColumnDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function Ts0CustomGrid.BeginRowDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function Ts0CustomGrid.EndColumnDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function Ts0CustomGrid.EndRowDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

procedure Ts0CustomGrid.CMShowingChanged(var Message: TMessage);
begin
  inherited;
  if Showing then UpdateScrollRange;
end;

function Ts0CustomGrid.IsDrawable(ACol, ARow: Integer): Boolean;
begin
  Result := True;
end;

procedure Ts0CustomGrid.InplaceEditMove(ACol, ARow: Integer);
begin
//  InplaceEditor.Move(CellRect(Col, Row));
end;

function Ts0CustomGrid.GetInplaceEditText(AControl: TWinControl): string;
begin
  Result := '';
end;

procedure Ts0CustomGrid.SetInplaceEditText(AControl: TWinControl;
  const Value: string);
begin
end;

function Ts0CustomGrid.GetInplaceEdit: TWinControl;
begin
  Result := nil;
end;

procedure Ts0CustomGrid.InplaceEditorMoveTo(ACol, ARow: Integer);
begin
  FInplaceCol := ACol;
  FInplaceRow := ARow;
end;

procedure Ts0CustomGrid.SetGridLineColor(const Value: TColor);
begin
...
function Ts0CustomDrawGrid.CellRect(ACol, ARow: Longint): TRect;
begin
  Result := inherited CellRect(ACol, ARow);
end;

procedure Ts0CustomDrawGrid.MouseToCell(X, Y: Integer; var ACol, ARow: Longint);
var
  Coord: TGridCoord;
begin
...
procedure Ts0CustomDrawGrid.ColumnMoved(FromIndex, ToIndex: Longint);
begin
  if Assigned(FOnColumnMoved) then FOnColumnMoved(Self, FromIndex, ToIndex);
end;

function Ts0CustomDrawGrid.GetEditMask(ACol, ARow: Longint): string;
begin
  Result := '';
  if Assigned(FOnGetEditMask) then FOnGetEditMask(Self, ACol, ARow, Result);
end;

function Ts0CustomDrawGrid.GetEditText(ACol, ARow: Longint): string;
begin
  Result := '';
  if Assigned(FOnGetEditText) then FOnGetEditText(Self, ACol, ARow, Result);
end;

procedure Ts0CustomDrawGrid.RowMoved(FromIndex, ToIndex: Longint);
begin
  if Assigned(FOnRowMoved) then FOnRowMoved(Self, FromIndex, ToIndex);
end;

function Ts0CustomDrawGrid.SelectCell(ACol, ARow: Longint): Boolean;
begin
  Result := True;
  if Assigned(FOnSelectCell) then FOnSelectCell(Self, ACol, ARow, Result);
end;

procedure Ts0CustomDrawGrid.SetEditText(ACol, ARow: Longint; const Value: string);
begin
  if Assigned(FOnSetEditText) then FOnSetEditText(Self, ACol, ARow, Value);
end;

procedure Ts0CustomDrawGrid.DrawCell(ACol, ARow: Longint; ARect: TRect;
...
procedure Ts0CustomDrawGrid.TopLeftChanged;
begin
...
{ Sparse array classes for Ts0CustomStringGrid }
...
constructor TStringGridStrings.Create(AGrid: Ts0CustomStringGrid; AIndex: Longint);
begin
  inherited Create;
  FGrid := AGrid;
  FIndex := AIndex;
...
constructor Ts0CustomStringGrid.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  Initialize;
end;

destructor Ts0CustomStringGrid.Destroy;
...
procedure Ts0CustomStringGrid.ColumnMoved(FromIndex, ToIndex: Longint);
...
procedure Ts0CustomStringGrid.RowMoved(FromIndex, ToIndex: Longint);
begin
...
function Ts0CustomStringGrid.GetEditText(ACol, ARow: Longint): string;
begin
  Result := Cells[ACol, ARow];
  if Assigned(FOnGetEditText) then FOnGetEditText(Self, ACol, ARow, Result);
end;

procedure Ts0CustomStringGrid.SetEditText(ACol, ARow: Longint; const Value: string);
begin
  DisableEditUpdate;
  try
...
procedure Ts0CustomStringGrid.DrawCell(ACol, ARow: Longint; ARect: TRect;
  AState: TGridDrawState);
begin
...
procedure Ts0CustomStringGrid.DisableEditUpdate;
begin
  Inc(FEditUpdate);
end;

procedure Ts0CustomStringGrid.EnableEditUpdate;
begin
  Dec(FEditUpdate);
end;

procedure Ts0CustomStringGrid.Initialize;
var
  quantum: TSPAQuantum;
begin
  if FCols = nil then
...
procedure Ts0CustomStringGrid.SetUpdateState(Updating: Boolean);
begin
  FUpdating := Updating;
...
procedure Ts0CustomStringGrid.Update(ACol, ARow: Integer);
begin
...
function  Ts0CustomStringGrid.EnsureColRow(Index: Integer; IsCol: Boolean):
  TStringGridStrings;
var
  RCIndex: Integer;
  PList: ^TSparseList;
...
function  Ts0CustomStringGrid.EnsureDataRow(ARow: Integer): Pointer;
var
  quantum: TSPAQuantum;
begin
...
function Ts0CustomStringGrid.GetCells(ACol, ARow: Integer): string;
var
...
function Ts0CustomStringGrid.GetCols(Index: Integer): TStrings;
begin
  Result := EnsureColRow(Index, True);
end;

function Ts0CustomStringGrid.GetObjects(ACol, ARow: Integer): TObject;
var
  ssl: TStringSparseList;
begin
  ssl := TStringSparseList(TSparseList(FData)[ARow]);
...
function Ts0CustomStringGrid.GetRows(Index: Integer): TStrings;
begin
  Result := EnsureColRow(Index, False);
end;

procedure Ts0CustomStringGrid.SetCells(ACol, ARow: Integer; const Value: string);
begin
...
procedure Ts0CustomStringGrid.SetCols(Index: Integer; Value: TStrings);
begin
  EnsureColRow(Index, True).Assign(Value);
end;

procedure Ts0CustomStringGrid.SetObjects(ACol, ARow: Integer; Value: TObject);
begin
...
procedure Ts0CustomStringGrid.SetRows(Index: Integer; Value: TStrings);
begin
