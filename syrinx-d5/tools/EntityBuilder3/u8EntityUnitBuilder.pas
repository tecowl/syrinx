...
type
  T8EntityUnitBuilder = class(Ts8EntityUnitWriter)
  private
    FPropID: T0SXMetaProperty;
    FPropCaption: T0SXMetaProperty;
    FIDConstName: string;
    FEtyClassCaption: string;
...
begin
  ...
  mthd.ImplMethod.Comment := MetaUnit.Comments.AddWithStrs([
  ...
  mthd.Visibility := k0cpvPublic;
  mthd.ImplMethod.ImplementCode.Add(                                      
    Format('  Result := ''%s'';', [FEtyClassCaption])
  );
  mthd.ImplMethod.Comment := MetaUnit.Comments.AddWithStrs([
...
procedure T8EntityUnitBuilder.AddInputProps(AClass: T0SXMetaClass; AProps: Ts0CollectionItemList);
  ...
  begin
    ...
    begin
      ...
      elem.Comment := MetaUnit.Comments.Add( AProp.EnumMember.Values[ elem.Name ]);
    end;
    ...
    Result.Collection := MetaUnit.Types;       
  end;
  //
...
begin
  ...
  begin
    ...
    begin
      ...
      fld.Comment := MetaUnit.Comments.Add(p.JPName);
    end
    else
    begin
      pr := 'F' + pn;
...
begin
  ...
  begin
    ...
    p := ProtoTypes.FindByName(prop.Name);
    ...
    mthd.ImplMethod.Comment :=
      MetaUnit.Comments.Add(prop.Name + 'プロパティのwriteメソッドです。');
    cd := mthd.ImplMethod.ImplementCode;
    cd.Clear;
...
begin
  ...
  begin
    ...
    if mthd.ImplMethod = nil then Continue;
    mthd.ImplMethod.Comment :=
      MetaUnit.Comments.Add(prop.Name + 'プロパティのreadメソッドです。');
    cd := mthd.ImplMethod.ImplementCode;
    cd.Clear;
    if (p <> nil) and p.IsPersistent then
    begin
...
begin
  ...
  cls.Name := FEtyClassName;
  cls.SuperClassName := 'Ts0Entity';
  cls.Collection := MetaUnit.Types;
  ...
  MetaUnit.InitializationCode.Add('  Ts0EntityRegister.RegisterEntity(' +  cls.Name + ');' );
  //
  InitUnit;
  WritePas;
end;

procedure T8EntityUnitBuilder.InitByMetaEntity(AEntity: T8MetaEntity);
begin
  FDestPas := AEntity.Files.EntityUnit.PasLines;
  FIDConstName := AEntity.IDConstName;
  FEtyClassName := AEntity.EtyClassName;
  FEtyClassCaption := AEntity.EtyClassCaption;
  if AEntity.OwnerComponent is Tc8MetaEntityMan then
    FIDDefineUnitName := Tc8MetaEntityMan(AEntity.OwnerComponent).IDDefineUnitName;
end;

procedure T8EntityUnitBuilder.InitUnit;
begin
end;
