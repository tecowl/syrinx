...
begin
  ...
  s0Database1.AliasName := FMetaEntityMan.BDEAliasName;
  s0Database1.DatabaseName := FMetaEntityMan.BDEAliasName;
...
procedure Tfrm8EBMain.actSelectTablesExecute(Sender: TObject);
  ...
  begin
    ...
    try
      ...
      tbl.TableName := AEntity.TableName;
...
begin
  ...
  try
    ...
    frm.BDEAliasName := FMetaEntityMan.BDEAliasName;
    if frm.ShowModal <> mrOK then Exit;
    FMetaEntityMan.BDEAliasName := frm.BDEAliasName;
    ...
    try
      sl.Assign(frm.Dest);
        ...
        begin
          ...
          ety.SourceExists := False;
          ety.TableExists := True;
          LoadProtoFromDS(ety);
        end;
    finally
      FreeAndNil(sl);
...
procedure Tfrm8EBMain.actSaveAsCSVExecute(Sender: TObject);
  ...
  begin
    ...
    try
      ite := Ts0CollectionIterator.Create(ACollection);
      try
