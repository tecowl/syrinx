...
type
  E8SQLGeneratorException = class(Exception);
...
begin
  ...
  begin
    Ts0GlobalValidationManager.Instance.NewMsg(
      'IDとして指定されたプロパティがありませんので、SQLのDelete文は生成されません。',
      k0vrWarning
    );
    Exit;
  end;
...
begin
  ...
  begin
    Ts0GlobalValidationManager.Instance.NewMsg(
      'IDとして指定されたプロパティがありませんので、単一行を選択するSelect文のWhere節を生成されません。',
      k0vrWarning
    );
    Exit;
  end;
...
begin
  ...
  begin
    Ts0GlobalValidationManager.Instance.NewMsg(
      'IDとして指定されたプロパティがありませんので、SQLのUpdate文は生成されません。',
      k0vrWarning
    );
    Exit;
  end;
...
{
resourcestring
  c9ErrMsg_IDPropNotExists = 'IDとしてしていされたプロパティが存在しません。';
}
procedure T8SQLGenerator.SetSource(const Value: Ts0EntityPropProtos);
begin
//  if (Value <> nil) and (Value.IDProp = nil) then
//    raise E8SQLGeneratorException.Create(c9ErrMsg_IDPropNotExists);
  FSource := Value;
end;
