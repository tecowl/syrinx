...
begin
  ...
  //
  grdEnumValue.Target := FDMBuilder.EnumProtos;
...
begin
  FTargetEntity := Value;
  pageEntityPas.Caption     := ChangeFileExt(FTargetEntity.EtyUnitName, '.pas');
  pageDMPas.Caption         := ChangeFileExt(FTargetEntity.DMUnitName, '.pas');
  pageDMDfm.Caption         := ChangeFileExt(FTargetEntity.DMUnitName, '.dfm');
  pageDetailFormPas.Caption := ChangeFileExt(FTargetEntity.DetailFormUnitName, '.pas');
  pageDetailFormDfm.Caption := ChangeFileExt(FTargetEntity.DetailFormUnitName, '.dfm');
  pageListFormPas.Caption   := ChangeFileExt(FTargetEntity.ListFormUnitName, '.pas');
  pageListFormDfm.Caption   := ChangeFileExt(FTargetEntity.ListFormUnitName, '.dfm');
  //
...
begin
  ...
  FDMBuilder.SQLGenerator.TableName := FTargetEntity.TableName;
...
var
  ...
  begin
    ...
    variable.Name := FTargetEntity.DMName;
    variable.TypeName := 'T' + FTargetEntity.DMName;
  ...
  begin
    AUnit.InitializationCode.Add(
      Format('  RegisterDispClass(T%s);', [FTargetEntity.DetailFormName])
    ); 
    AUnit.FinalizationCode.Add(
      Format('  UnRegisterDispClass(T%s);', [FTargetEntity.DetailFormName])
    );
    variable := T0SXMetaVariable(AUnit.Variables.Add);
    variable.Name := FTargetEntity.DetailFormName;
    variable.TypeName := 'T' + FTargetEntity.DetailFormName;
    AUnit.ImplComment := AUnit.Comments.Add('{$R *.DFM}');
    AUnit.ImplComment.CommentKind := k0ckCustom;
    AUnit.UsesIntf.Add( 'Windows' );
    AUnit.UsesIntf.Add( 'Messages' );
    ...
    AUnit.UsesIntf.Add( FTargetEntity.EtyUnitName );
  ...
  begin
    AUnit.InitializationCode.Add(
      Format('  RegisterDispClass(T%s);', [FTargetEntity.ListFormName])
    );
    AUnit.FinalizationCode.Add(
      Format('  UnRegisterDispClass(T%s);', [FTargetEntity.ListFormName])
    );
    variable := T0SXMetaVariable(AUnit.Variables.Add);
    variable.Name := FTargetEntity.ListFormName;
    variable.TypeName := 'T' + FTargetEntity.ListFormName;
    AUnit.ImplComment := AUnit.Comments.Add('{$R *.DFM}');
    AUnit.ImplComment.CommentKind := k0ckCustom;
    AUnit.UsesIntf.Add( 'Windows' );
    AUnit.UsesIntf.Add( 'Messages' );
...
begin
  ...
  try
    ...
    try
      ...
      unitEntity := FProdProj.Units.AddUnit( FTargetEntity.EtyUnitName );
      unitDM := FProdProj.Units.AddUnit( FTargetEntity.DMUnitName );
      unitList := FProdProj.Units.AddUnit( FTargetEntity.ListFormUnitName );
      unitDetail := FProdProj.Units.AddUnit( FTargetEntity.DetailFormUnitName );
...
begin
  ...
  s0Table1.TableName := TargetEntity.TableName;
